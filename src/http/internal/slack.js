...
export default ({ app }) => {
    ...
    slackInteractions.action({ callbackId: "review" }, async (payload, respond) => {
        ...
        if (payload.actions[0].name === "Reject invocation name") {
            ...
            return alexa.updateForUsername(payload.actions[0].value, { status: "rejected", rejectReason: "The invocation name text does not comply to Amazon's specifications" })
        }

        if (payload.actions[0].name === "Reject intro") {
            await slack.disableButtons(payload.original_message, `:heavy_multiplication_x: rejected by <@${payload.user.id}>`)
            return alexa.updateForUsername(payload.actions[0].value, { status: "rejected", rejectReason: "The intro text does not comply to Amazon's specifications" })
        }

        if (payload.actions[0].name === "Reject help") {
            await slack.disableButtons(payload.original_message, `:heavy_multiplication_x: rejected by <@${payload.user.id}>`)
            return alexa.updateForUsername(payload.actions[0].value, { status: "rejected", rejectReason: "The help text does not comply to Amazon's specifications" })
        }
        ...
        if (payload.actions[0].name === "Accept") {
            ...
            return alexa.updateForUsername(payload.actions[0].value, { status: "approved" })
        }
    });
}
