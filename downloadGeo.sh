...
wget https://download.maxmind.com/app/geoip_download?edition_id=GeoLite2-Country&license_key=${GEOLITE_KEY}&suffix=tar.gz -O GeoLite2-Country.tar.gz || { echo 'Could not download GeoLite2 Country, exiting.' ; exit 1; }
wget https://download.maxmind.com/app/geoip_download?edition_id=GeoLite2-City&license_key=${GEOLITE_KEY}&suffix=tar.gz -O GeoLite2-City.tar.gz || { echo 'Could not download GeoLite2 City, exiting.' ; exit 1; }
wget https://download.maxmind.com/app/geoip_download?edition_id=GeoLite2-ASN&license_key=${GEOLITE_KEY}&suffix=tar.gz -O GeoLite2-ASN.tar.gz || { echo 'Could not download GeoLite2 ASN, exiting.' ; exit 1; }
